...
def inferencing_threadpool(
    ...
    def worker_task(queue_index):
        ...
        model.clean()
        print(
            f"线程池工作线程 {thread_name} (队列 {queue_index}) 正常退出，共处理 {frame_count} 帧"
        )
        return queue_index, frame_count, thread_name

    executor = concurrent.futures.ThreadPoolExecutor(
        max_workers=len(read_queues), thread_name_prefix="InferPool"
    )
...
def inferencing_threadpool_with_callback(
    read_queues: list, result_queues: list, model_path: str, stop_event: threading.Event
):
    def worker_task(queue_index):
        read_queue = read_queues[queue_index]
    ...
    def task_done_callback(future):
        try:
            ...
            print(
                f"回调: 线程 {thread_name} (队列 {queue_index}) 处理完成，共处理 {frame_count} 帧"
            )
...
def _process_worker(queue_index, read_queue, result_queue, model_path, stop_event):
    ...
    model.clean()
    print(
        f"进程池工作进程 {process_name} (队列 {queue_index}) 正常退出，共处理 {frame_count} 帧"
    )
...
def inferencing_processpool(read_queues: list, result_queues: list, model_path: str, stop_event: mp.Event):  # type: ignore
    executor = concurrent.futures.ProcessPoolExecutor(max_workers=len(read_queues))

    futures = []
    for i in range(len(read_queues)):
        future = executor.submit(
            _process_worker, i, read_queues[i], result_queues[i], model_path, stop_event
        )
        futures.append(future)
...
def inferencing_processpool_with_callback(read_queues: list, result_queues: list, model_path: str, stop_event: mp.Event):  # type: ignore
    def task_done_callback(future):
        try:
            ...
            print(
                f"回调: 进程 {process_name} (队列 {queue_index}) 处理完成，共处理 {frame_count} 帧"
            )
        except Exception as e:
            print(f"回调: 任务执行出错: {e}")
